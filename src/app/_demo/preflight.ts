/**
 * Deterministic preflight evaluator for demo mode.
 *
 * The real `/api/preflight` route owns the production checks. This module
 * exists so demo mode can honestly react to the judge's edits instead of
 * replaying a canned verdict: every rule below is a transparent, deterministic
 * measurement of the submitted draft, and the output is validated against the
 * shared response schema before it reaches the UI.
 */
import {
...
/** On-screen scene copy longer than this reads poorly at Short/slide pace. */
...
function checkAudienceFit(pack: PreflightDraftContentPack): PreflightCheck {
...
function checkBrandSafety(pack: PreflightDraftContentPack): PreflightCheck {
  const corpus = fullText(pack);
...
export function evaluateDraftPack(
